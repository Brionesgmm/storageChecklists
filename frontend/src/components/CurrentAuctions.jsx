...
const CurrentAuctions = ({
  ...
  const addUnit = (auctionId) => {
    const unitToAdd = newUnits[auctionId];
    if (unitToAdd) {
      setCurrentAuctions((prevAuctions) => {
        const newAuctions = [...prevAuctions];
        const auctionIndex = newAuctions.findIndex(
        ...
        newAuctions[auctionIndex].auctionUnits.push({
          unit: unitToAdd,
          id: `${Date.now()}`,
        });
        return newAuctions;
  ...
  const handleDeleteUnit = (auctionId, unitId) => {
    setIsMakingChanges(true);
    setIsDataSubmitted(false);
    setCurrentAuctions((prevAuctions) => {
      const newAuctions = [...prevAuctions];
                      ...
                      {auction.auctionUnits.map((unit) => (
                        <div className="units" key={unit.id}>
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              handleDeleteUnit(auction.id, unit.id)
                            }
                          >
                            Delete 🗑️
                          </button>
